...
class CreatePropertyBloc extends Bloc<CreatePropertyEvent, CreatePropertyState> {
  ...
  Future<void> _onSubmitted(CreatePropertySubmitted event, Emitter<CreatePropertyState> emit) async {
    emit(const CreatePropertyLoadInProgress());
    final result = await _createPropertyUsecase(CreatePropertyParams(
      name: event.name, address: event.address, ward: event.ward,
