...
@injectable
class DeletePropertyBloc
    extends Bloc<DeletePropertyEvent, DeletePropertyState> {
  ...
  Future<void> _onSubmitted(
    DeletePropertySubmitted event,
    Emitter<DeletePropertyState> emit,
  ) async {
    emit(const DeletePropertyLoadInProgress());
    final result = await _deletePropertyUsecase(
      DeletePropertyParams(id: event.id),
    );
    result.fold(
