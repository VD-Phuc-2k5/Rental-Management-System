...
import { RoomRepository, CreateRoomInput, UpdateRoomInput } from "../domain/repositories/room.repository";
...
export class DrizzleRoomRepository implements RoomRepository {
    ...
    async createRoom(input: CreateRoomInput): Promise<RoomEntity> {
        const [row] = await this.drizzleService.db.insert(rooms)
            .values({
    ...
    async findAllByPropertyId(propertyId: string): Promise<RoomEntity[]> {
        const rows = await this.drizzleService.db.select().from(rooms)
            .where(eq(rooms.propertyId, propertyId));
        return rows.map(r => this.toEntity(r));
    }

    async findById(id: string): Promise<RoomEntity | null> {
        const [row] = await this.drizzleService.db.select().from(rooms)
    ...
    async updateRoom(id: string, input: UpdateRoomInput): Promise<RoomEntity> {
        ...
        if (input.area_sqm !== undefined) updateData.area_sqm = String(input.area_sqm);
        if (input.monthly_rent !== undefined) updateData.monthly_rent = String(input.monthly_rent);
        if (input.deposit_amount !== undefined) updateData.deposit_amount = String(input.deposit_amount);
        if (input.electricity_rate_per_kwh !== undefined) updateData.electricity_rate_per_kwh = String(input.electricity_rate_per_kwh);
        if (input.water_rate_per_m3 !== undefined) updateData.water_rate_per_m3 = String(input.water_rate_per_m3);
        if (input.has_furniture !== undefined) updateData.has_furniture = input.has_furniture;
        if (input.description !== undefined) updateData.description = input.description;

        const [row] = await this.drizzleService.db.update(rooms)
            .set(updateData).where(eq(rooms.id, id)).returning();
        return this.toEntity(row);
    }
